...
int main(int argc, char **argv) {
    ...
    // Time evolution operatror Exp(delta_t): one complex kernel
    mrcpp::TimeEvolutionOperator<1> Exp(MRA, prec, delta_t, finest_scale);
    println(0, Exp.getComponent(0, 0));
    ...
    auto integral = error.integrate();
    auto sq_norm = error.getSquareNorm();
    mrcpp::print::value(0, "Integral of    Exp(delta_t) f(x) - g(x)  (real part) =", integral.real());
    mrcpp::print::value(0, "Integral of    Exp(delta_t) f(x) - g(x)  (imag part) =", integral.imag());
    mrcpp::print::value(0, "Square norm of Exp(delta_t) f(x) - g(x)              =", sq_norm);
    ...
    mrcpp::Coord<1> o{0.0};
    mrcpp::Coord<1> a{1.0};
    mrcpp::Plotter<1, ComplexDouble> plot(o);
    plot.setRange(a);

    plot.linePlot({nPts}, error, "error");   // Write to file error.line
    plot.linePlot({nPts}, f_tree, "f_tree"); // Write to file f_tree.line
    plot.linePlot({nPts}, g_tree, "g_tree"); // Write to file g_tree.line
